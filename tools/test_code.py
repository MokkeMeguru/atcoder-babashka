...
if __name__ == '__main__':
    ...
    for idx, (input_data, output_data) in enumerate(zip(inputs_dir.iterdir(), outputs_dir.iterdir())):
        print("Test Case {} :".format(idx), end="")
        with input_data.open() as fr, tempfile.NamedTemporaryFile() as fw, output_data.open() as fro:
            subprocess.run(run_command +[str(source_file.absolute())], stdin=fr, stdout=fw, check=True)
            ...
            if result != ans:
                ...
                print("failed", emoji.emojize(":thumbsdown:" , use_aliases=True))
                print(''.join(diff), end="")
            else:
                print("passed", emoji.emojize(":+1:" , use_aliases=True))
